...
ContentOS now supports the core owner post workflow, a complete owner media library (direct R2 uploads with verified generated preview variants, search, editor insertion/reuse, and reference-safe deletion), a CORS-gated public JSON feed of published posts, the settings-backed appearance slice, the broad owner settings form (identity, timezone, model, writing profile, Umami URL, env-backed storage readout), the owner-only Umami analytics surface, and streamed AI draft generation from the saved writing profile. Deployment verification, publishing toggles without consumers, social repurposing, scheduling, and optional community features remain incomplete.
