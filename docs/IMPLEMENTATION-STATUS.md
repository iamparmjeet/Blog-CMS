...
- The Drizzle journal is a single regenerated baseline from `full-schema.ts` plus additive milestones through `0007_optimal_switch.sql` (0005 rate limits, 0006 default-model default, 0007 backup restore markers); the `todos` scaffold table is gone and the baseline applies from an empty local D1 (`0000_small_scourge.sql`). Migration 0007 has been applied locally, not remotely.
