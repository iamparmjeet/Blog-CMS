...
| Backup/restore | Implemented locally, not deployed | Owner-only version-1 JSON export includes all posts, settings, activity, and R2 objects. Settings → Storage → Danger zone offers Merge or explicitly confirmed Replace; new keys, slugs, post/media IDs, and embedded references are remapped in one D1 batch. Backups over 16 MiB stream one media object per request through R2, then commit a bounded metadata manifest; migration 0007 records committed session IDs for idempotent retries. Missing originals are rejected; older v1 exports without IDs remap by URL. See `docs/backup.md` and both backup E2E artifacts. |
| Deployment | Implemented (M1.3) | D1 (`DB`) and R2 (`MEDIA`) bindings are declared; local development uses local D1 and the remote `contentos` R2 bucket for the direct-upload verification path. Remote D1 `blog-cms` (`142c33e3-399b-4eea-9164-10995ce4f115`) is provisioned through migration 0006; the new backup-restores migration 0007 is local-only until this slice deploys. `wrangler.jsonc` carries the real id, and `docs/deploy.md` + `bun run deploy:check` document/validate the preview and production flow. Preview is live at `https://contentos.parmjeetmishra.com` (canonical; `https://blog-cms.parmjeetmishra.workers.dev` stays up for non-auth smoke checks): `/` serves 200, `/dashboard` 307-redirects to login when signed out, `/api/posts` returns the documented payload from live D1 and 403s disallowed origins. Worker boots with no native SQLite dependencies. |
...
- The Drizzle journal is a single regenerated baseline from `full-schema.ts` plus additive milestones through `0006_certain_mongoose.sql` (0005 rate limits, 0006 default-model default); the `todos` scaffold table is gone and the baseline applies from an empty local D1 (`0000_small_scourge.sql`).
...
`origin/main` is at `500b7e8` (PR #32 docs rollup); the backup-import slice lives on `feat/backup-chunked-restore` until merged. M0–M6.1 are all checked. Earlier tickets landed as one slice PR from a fresh branch based on current `main`:
...
- Backup import/restore is in `feat/backup-chunked-restore` (not yet merged/deployed): Merge and Replace, including a chunked path for large backups. One-shot export/import and chunked E2E artifacts are in `docs/e2e/`. Migration 0007 adds `backup_restores` for idempotent finalization; applied to local D1, not remote. Checks: 290/290 tests, types, Biome (213 files), build, and three backup E2Es all green.
- Next: review and merge the slice PR after CI, apply migration 0007 remotely, then redeploy and smoke-test the canonical Worker. `deploy:preview` and `deploy` share the same Worker, not isolated environments (`docs/deploy.md`). Do not exercise Replace against the live owner instance as a smoke test. Workers PR-branch builds remain a dashboard-side issue (bun-only repo). M6.2 stays gated behind a D8 revisit.
