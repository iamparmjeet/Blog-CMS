...
- **Copy and setup.** The terminal shows `git clone https://github.com/iamparmjeet/PageOwl.git`, `cd PageOwl && bun install`, `bun run db:migrate:remote`, and `bun run deploy`, with a deploy-guide link. The hero and final CTAs say “Self-host it free” and point at the open-source setup section instead of owner login/dashboard.
...
`main` is 7 commits ahead of `origin/main` (worktree clean) and production serves the marketing release. The follow-on slices are committed but **not pushed and not deployed** as of 2026-09-23:

- Marketing follow-up (`800175e`) + release record (`290a0cd`): mobile menu, contrast-safe accents, corrected self-host copy, mobile product preview, footer stacking. Deployed as version `14132947-55dd-40f6-ae7a-06a1b45bbf0c` after migration 0008.
- Title-following draft slugs (`d290c1c`): `slugForTitle`/`slugFollowsTitle`, `selectOwnerSlugsExcluding`, `publishedAt` in the editor payload, test-first units plus `scripts/auto-slug-e2e.ts`.
- Responsive footer alignment (`3793ce4`).
- Live demo sandbox (`a0f94f4`, `c650c9f`): public `/demo` with dashboard, posts, editor, analytics, and settings views; deterministic sample data; simulated repurposing; linked from the header, hero, and login.
- Responsive fixes (`73a1d54`): settings tabs and the demo view switcher wrap on narrow screens.

Next release step: `git push` (pre-push hooks run typecheck/lint/build) and `bun run deploy`; no migrations are pending.
