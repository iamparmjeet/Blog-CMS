...
| --- | --- | --- |
| Marketing site | Implemented, visual follow-up open | Landing sections include hero, repurpose, how-it-works, open-source callout, and final CTA. The decorative product mock is responsive, `aria-hidden`/`inert`, and motion respects reduced-motion preferences. The PageOwl mark appears in the header and footer; the footer starts animating on entry. Mobile public navigation, copy/setup accuracy, and accent contrast remain follow-ups. |
...
| `bun run check` | Passes | Biome 2.4.5 clean on 214 files for the local PageOwl change. |
...
Marketing follow-ups from the review remain open: the public mobile menu is currently inert; default/custom brand buttons and light muted text need contrast-safe values; the setup snippet contains an unavailable `db:push` command and an invalid shorthand clone URL; “Get started free” leads to owner login rather than self-host setup; and the mobile mock hides the repurpose panel. Platform-specific dots and green success states are intentional semantic exceptions to the blue brand palette.
