...
- Next: diagnose the failed Cloudflare Workers Builds checks on PR branches using the dashboard build logs; GitHub check-run summaries only show failure, while the corresponding `main` builds succeeded. `deploy:preview` and `deploy` share the same Worker, not isolated environments (`docs/deploy.md`). M6.2 stays gated behind a D8 revisit.
